...
using WeightsCoeff = std::vector<float>;
...
struct VertexStruct
{
	...
	std::vector<Vertex> neightbors;

	VertexStruct(const unsigned int n, const Weights& W = {1}, const WeightsCoeff& wCoeff = {1.f})
	{
...
static void connect(Vertex a, Vertex b)
{
	...
	//On connect deux sommets que s'ils ne sont pas d�j� connect�s
...
{
public:
	Graph(const Edges& edges = {}) : m_edges(std::move(edges)) { rebuildVerticesSet(); }
